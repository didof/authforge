...
This demo is a local API that wires AeonKey into Express with SQLite storage.
...
The server listens on `http://localhost:3000` by default and creates `aeonkey-example.sqlite` in this package directory unless `AEONKEY_EXAMPLE_DB` is set.
...
```sh
CSRF=$(curl -s -c /tmp/aeonkey-cookies http://localhost:3000/csrf | node -e 'process.stdin.on("data", d => process.stdout.write(JSON.parse(d).csrfToken))')
```

Use the token in every POST:

```sh
-H "x-csrf-token: $CSRF" -b /tmp/aeonkey-cookies -c /tmp/aeonkey-cookies
```

## Signup

```sh
curl -s -b /tmp/aeonkey-cookies -c /tmp/aeonkey-cookies \
...
```sh
curl -s -b /tmp/aeonkey-cookies http://localhost:3000/auth/session
```
